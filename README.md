...
- **Noctalia v5** como shell (repo APT `pkg.noctalia.dev`, suite `trixie` en Debian 13 / `trixie→unstable` con fallback en Debian 14 `forky`, no existe suite `forky` a 2026-09; si ninguna suite instala, el módulo avisa y **sigue** — Hyprland queda usable y `99-final-check` marca `COMPLETADA CON ADVERTENCIA`): barra solo en laptop (`noctalia/bar-monitors.toml`), arranque con `exec-once = noctalia`, binds IPC (`SUPER+Space` launcher, `SUPER+O` control-center, `SUPER+comma` settings, `ALT + Tab` switcher). Reintento: `sudo ./install.sh --only 71-noctalia,99-final-check`. Guía en `NOCTALIA_COMANDOS.md`
