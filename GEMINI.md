...
Este proyecto utiliza Hyprland sobre una base de Debian con GNOME instalado. Para garantizar la compatibilidad y funcionalidad de los módulos de Eww, se deben seguir las siguientes convenciones:

## Herramientas de Gestión

- **Red (WiFi/Ethernet/VPN):** Utilizar `nmtui` ejecutado en la terminal `foot` para la gestión de conexiones. 
    - Se utiliza un script personalizado (`~/.config/hypr/waybar_network.sh`) para unificar los iconos de WiFi y VPN en la barra.
...
- **Bluetooth:** Utilizar `blueman-manager` para la gestión gráfica de dispositivos. El módulo de la barra está configurado para ser minimalista, mostrando solo iconos de estado (Conectado/Encendido/Apagado) y ocultando el nombre del dispositivo en el tooltip.
...
3. Reiniciar la barra con `Super + Shift + B`.
